    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('templates.store') }}" class="p-6 space-y-5">
                    @csrf

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Nama template</label>
                        <input type="text" name="name" value="{{ old('name') }}" required class="mt-1 border-gray-300 rounded-md text-sm w-full" />
                        @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Deskripsi (opsional)</label>
                        <textarea name="description" rows="2" class="mt-1 border-gray-300 rounded-md text-sm w-full">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="table_layout" rows="6" class="mt-1 border-gray-300 rounded-md text-sm w-full font-mono text-xs" placeholder='{"columns":[{"type":"row_number","label":"No","rowspan":2},{"type":"group","label":"Nama","colspan":2,"children":[{"type":"field","field":"Nama Suami","label":"Suami"},{"type":"field","field":"Nama Istri","label":"Istri"}]}]}'>{{ old('table_layout') }}</textarea>
                    ...
                    <div>
                        ...
                        <select onchange="window.location.href='{{ route('templates.create') }}?source_import_id='+this.value" class="mt-1 border-gray-300 rounded-md text-sm w-full sm:w-auto">
                            <option value="">-- Pilih file --</option>
                            @foreach ($imports as $import)
                                <option value="{{ $import->id }}" @selected($sourceImport?->id === $import->id)>{{ $import->file_name }}</option>
                            @endforeach
                        </select>
                    </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                @foreach ($columns as $column)
                                    <label class="flex items-center gap-2 text-sm text-gray-700 border border-gray-200 rounded-md px-3 py-2 cursor-pointer hover:border-blue-400">
                                        <input type="checkbox" name="fields[]" value="{{ $column }}" @checked(in_array($column, old('fields', $columns))) class="rounded text-blue-600" />
                                        {{ $column }}
                                    </label>
                                @endforeach
                            </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" name="search" value="{{ old('search') }}" class="mt-1 border-gray-300 rounded-md text-sm w-full" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Filter kolom (default)</label>
                            <input type="text" name="filter_column" value="{{ old('filter_column') }}" class="mt-1 border-gray-300 rounded-md text-sm w-full" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Nilai filter (default)</label>
                            <input type="text" name="filter_value" value="{{ old('filter_value') }}" class="mt-1 border-gray-300 rounded-md text-sm w-full" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="sort_column" value="{{ old('sort_column') }}" class="mt-1 border-gray-300 rounded-md text-sm w-full" />
                        </div>
                        <div>
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-md hover:bg-blue-700">Simpan Template</button>
                    </div>
                </form>
            </div>
        </div>
