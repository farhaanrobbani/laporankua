<x-admin.layout>
    <x-slot name="header">Ubah Template</x-slot>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <form method="POST" action="{{ route('admin.templates.update', $template) }}" class="p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $template->name) }}" required class="mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md text-sm w-full" />
                @error('name') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Deskripsi (opsional)</label>
                <textarea name="description" rows="2" class="mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md text-sm w-full">{{ old('description', $template->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="table_layout" rows="6" class="mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md text-sm w-full font-mono text-xs" placeholder='{"columns":[{"type":"row_number","label":"No","rowspan":2}]}'>{{ old('table_layout', !empty($template->layout_json['table_layout']) ? json_encode($template->layout_json['table_layout'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '') }}</textarea>
            ...
            <div>
                ...
                <select onchange="window.location.href='{{ route('admin.templates.edit', $template) }}?source_import_id='+this.value" class="mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md text-sm w-full sm:w-auto">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        @foreach ($columns as $column)
                            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-md px-3 py-2 cursor-pointer hover:border-blue-400 dark:hover:border-blue-500">
                                <input type="checkbox" name="fields[]" value="{{ $column }}" @checked(in_array($column, old('fields', $template->fields_json ?? []))) class="rounded text-blue-600 dark:text-blue-400" />
                                {{ $column }}
                            </label>
                        @endforeach
                    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="search" value="{{ old('search', $template->filters_json['search'] ?? '') }}" class="mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md text-sm w-full" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Filter kolom (default)</label>
                    <input type="text" name="filter_column" value="{{ old('filter_column', $template->filters_json['filter_column'] ?? '') }}" class="mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md text-sm w-full" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nilai filter (default)</label>
                    <input type="text" name="filter_value" value="{{ old('filter_value', $template->filters_json['filter_value'] ?? '') }}" class="mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md text-sm w-full" />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="sort_column" value="{{ old('sort_column', $template->sorting_json['column'] ?? '') }}" class="mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md text-sm w-full" />
                </div>
                <div>
            ...
            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                <input type="checkbox" name="is_default" value="1" @checked($template->is_default) class="rounded text-blue-600 dark:text-blue-400" />
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 dark:bg-blue-700 text-white text-sm font-semibold rounded-md hover:bg-blue-700 dark:hover:bg-blue-600">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</x-admin.layout>
